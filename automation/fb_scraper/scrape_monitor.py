...
def main():
    ...
    for i in range(1, 16):
        ...
        # Check log
        log_out, _ = ssh_exec(client, 'tail -10 /var/log/scraper_run.log 2>/dev/null || echo "(no log)"')
        print(f"Log tail:\n{log_out}")
...
" 2>&1"""
        ...
        if not scraper_running:
            ...
            if ps_join:
                ...
                print("WARNING: join_all.py not found in process list. Checking log...")
    ...
    print("\n*** Timeout: 15 checks done, scraper still running. ***")
    db_out, _ = ssh_exec(client, db_cmd)
    print(f"Current counts: {db_out}")
    client.close()
